...
function BindingData()
{
    ...
    var language = "<%=CtlLib.SetListDataFUNC("SELECT ac_GET_COMMONCODE('COAB0070') FROM DUAL")%>";
    var report = "<%=CtlLib.SetListDataFUNC("SELECT ac_GET_COMMONCODE('EACBK036') FROM DUAL")%>";
	...
	rdoSTATUS.value = l_status;
	var l_company = "<%=Request.querystring("company")%>";
	lstCompany.value = l_company;
	var l_from = "<%=Request.querystring("from")%>";
	txtFROM_DATE.value = l_from;
	var l_to = "<%=Request.querystring("to")%>";
	txtTO_DATE.value = l_to;
	
	var l_ac_pk = "<%=Request.querystring("ac_pk")%>";
	var l_ac_cd = "<%=Request.querystring("ac_cd")%>";
	var l_ac_nm = "<%=Request.querystring("ac_nm")%>";
	txtACPK.text = l_ac_pk;
	txtACCD.text = l_ac_cd;
	txtACNM.text = l_ac_nm;
	
	var l_pl_center = "<%=Request.querystring("pl_center")%>";
	var l_pl_center_nm = "<%=Request.querystring("pl_center_nm")%>";
	txtPLC_CD.text = l_pl_center;
	txtPLC_NM.text = l_pl_center_nm;
	
	var l_pl_unit = "<%=Request.querystring("pl_unit")%>";
	var l_pl_unit_nm = "<%=Request.querystring("pl_unit_nm")%>";
	txtPLCD.text = l_pl_unit;
	txtPLNM.text = l_pl_unit_nm;
...
function RowClick()
{
    ...
    if (rownum > 0) 
    {
        ...
        //{
            //ls_arr = ls_arr + ' ' + grdVoucher.GetGridData(i, 0);
        //}
        var fpath = System.RootURL + '/standard/forms/60/08/60080050_popup.aspx?PK_ROW=' + pk + '&ROW=' + ctrl.Row + '&ROWS=' + rows + '&APPRCANCEL=' + status + "&ARRAY_PK=" + ls_arr;
        var wWidth = 950;
        var wHeight = 600;
        var x = Math.round((screen.availWidth - wWidth)/2);
        var y = Math.round((screen.availHeight - wHeight)/2);
...
function OnPrint()
{            var url = '/standard/reports/60/08/rpt_60080020_SL_PL.aspx?p_compk=' +lstCompany.value + '&p_from=' + txtFROM_DATE.value + '&p_to=' + txtTO_DATE.value+ '&p_acc_pk='+txtACPK.text+ '&p_plcenter='+txtPLC_PK.text+ '&p_plunit='+txtPLPK.text+ '&p_status='+rdoSTATUS.value+ '&p_ccy='+lstCcy.value;
  ...
  <gw:data id="dat_gfka00220_1" onreceive=""  > 
   ...
   <gw:data id="dso_sel_PL_bal_sum" onreceive="OnDataReceive(this)">
        <xml>
            <dso type="grid" function="ac_sel_60080200_plsum_popup">
                <input bind="grdDetail">
                    <input bind="lstCompany"/>
                    <input bind="txtFROM_DATE"/>
                    <input bind="txtTO_DATE"/>
                    <input bind="txtACPK"/>
                    <input bind="txtPLC_CD"/>
					<input bind="txtPLC_NM"/>
                    <input bind="txtPLCD"/>
					<input bind="txtPLNM"/>
    ...
    <gw:data id="dso_date" onreceive="OnDataReceive(this)">
        <xml>
            <dso type="grid" function="ac_sel_60080200_1_popup">
                <input bind="grdDetail">
                    <input bind="lstCompany"/>
                    <input bind="txtFROM_DATE"/>
                    <input bind="txtTO_DATE"/>
    ...
    <gw:data id="dso_voucher" onreceive="OnDataReceive(this)">
        <xml>
            <dso type="grid" function="ac_sel_60080200_popup">
                <input bind="grdDetail">
                    <input bind="lstCompany"/>
                    <input bind="txtFROM_DATE"/>
                    <input bind="txtTO_DATE"/>
            ...
            <table style="width:100%;height:100%" cellpadding="0" cellspacing="0" border="0">
                ...
                <tr >
                    ...
                    <td ><gw:imgbtn img="excel" alt="Print" id="btnPrint11" onclick="OnPrint()" /></td>        
                </tr>
                <tr>
               ...
               <tr>
                    ...
                    <td colspan="3"  > <gw:radio id="rdoSTATUS"  value="0" > 
               ...
               <tr>
                    ...
                    <td  colspan="5"><gw:list id="lstREPORT" styles='width:100%'onclick="onPrint(2)"/></td>
               </tr>
            </table>     
                        </fieldset>
                   </td>
