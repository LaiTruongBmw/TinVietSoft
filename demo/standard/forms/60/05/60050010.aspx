...
function BindingDataList(){
    ...
    var ls_data4    = "<%=CtlLib.SetDataSQL("SELECT CODE FROM TAC_COMMCODE_MASTER A, TAC_COMMCODE_DETAIL B WHERE A.PK = B.TAC_COMMCODE_MASTER_PK AND A.ID = 'ACBG0130' AND B.CHAR_3 = 'RV' AND A.DEL_IF = 0 AND B.DEL_IF = 0")%>";
    var ls_data5    = "<%=CtlLib.SetDataSQL("SELECT NUM_2 FROM TAC_COMMCODE_MASTER A, TAC_COMMCODE_DETAIL B WHERE A.PK = B.TAC_COMMCODE_MASTER_PK AND A.ID = 'ACBG0130' AND B.CHAR_3 = 'RV' AND A.DEL_IF = 0 AND B.DEL_IF = 0")%>";

    var ls_data6    = "<%=CtlLib.SetDataSQL("SELECT NVL(NUM_3, '0') FROM TAC_COMMCODE_MASTER A, TAC_COMMCODE_DETAIL B WHERE A.PK = B.TAC_COMMCODE_MASTER_PK AND A.ID = 'ACBG0130' AND B.CHAR_3 = 'RV' AND A.DEL_IF = 0 AND B.DEL_IF = 0")%>";
    ...
    var ls_data8    = "<%=CtlLib.SetListDataSQL("SELECT D.CODE, D.CODE_NM FROM TAC_COMMCODE_MASTER M, TAC_COMMCODE_DETAIL D WHERE M.PK = D.TAC_COMMCODE_MASTER_PK AND M.ID = 'GFQC0016' and d.use_yn = 'Y' AND M.DEL_IF = 0 AND D.DEL_IF = 0 and d.remark = 'R'  ORDER BY D.CODE_NM")%>"; 
...
{    
	...
	{
		flg = "2";
		//var path = System.RootURL + '/standard/forms/60/05/60050010_popup_ItemControl.aspx';
		var path = System.RootURL + '/standard/forms/60/05/60050010_popup_4.aspx?com_pk=' + lstCompany.value;
		var object = System.OpenModal( path ,800 , 600 , 'resizable:yes;status:yes');
		if ( object != null )
		{
			...
			else
			{
				...
				else
				{
					...
					else
					{
						...
						{
							//var fpath   = System.RootURL + "/standard/forms/60/05/60050010_popup_3.aspx"; Original
...
function OnReport()
{
	if(txtseq.text!='')
	{
		...
		else if(lstReport.value=='1020_v2')
		{
			...
			{
				rqt = "?company_pk=" + lstCompany.GetData() + '&seq='+txtseq.GetData() + '&pv_pk=' + txtseq1.GetData()+'&circulars='+lstCirculars.value+'&print_amount='+lst_Print_Amount.value;
				url = System.RootURL + '/standard/reports/gf/br/gfbr00030_ReceiptVoucher_General_pdf_A5.aspx'+rqt;
			}
		}
...
function OnReportRV()
{
	...
	{
		rqt = "?company_pk=" + lstCompany.GetData() + '&seq=' + txtseq.GetData()+'&pv_pk=' + txtseq1.GetData();
		url =System.RootURL + '/standard/reports/gf/br/gfbr00030_ReceiptVoucher_General_excel.aspx' + rqt; 
		if(confirm("Are you sure you want to report ?" ))
		{
			System.OpenTargetPage( url , 'newform' );
			return;
...
function OnReportAMB()
{
	if(txtseq.GetData() != "")
	{
		var url =System.RootURL + '/standard/reports/60/05/60050010.aspx?seq='+txtseq.text  ;
