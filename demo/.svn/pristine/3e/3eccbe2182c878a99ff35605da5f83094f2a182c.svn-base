...
function OnDataReceive(obj)
{
   ...
   { 
     ...
     if(txtTemp.text=='3')
        ...
        {
            alert("Họ tên hoặc ngày sinh có trong danh sách từ chối!");
            check = "N";
            tmp =1;
        }
        ...
        {
            alert("Số CMND có trong danh sách từ chối!");
            check = "N";
            tmp =1;
        }
        
        if(tmp == 0 && CheckInfo()==1)
        {
...
function ChangeShool()
{
    if(lstEducation.value=="05" || lstEducation.value=="09") // MA or University
    {
        tr1.style.display ="none" ; // not
        tr2.style.display="block" ;   //  show  

        tr3.style.display ="none" ; // not
        tr4.style.display="block" ;   //  show 
        txtSchool.text = lstSchool.GetText();
    }
    else
    {
        tr1.style.display ="block" ; // show
        tr2.style.display="none" ;   // not show  

        tr3.style.display ="block" ; // show
        tr4.style.display="none" ;   // not show  
    }
...
<gw:data id="datCheck" onreceive="OnDataReceive(this)"> 
        <xml> 
            <dso  type="process" procedure="HR.sp_pro_check_block" > 
                <input>
...
<gw:data id="datFindRec" onreceive="OnDataReceive(this)"> 
        <xml> 
            <dso  type="process" procedure="hr.sp_pro_find_rec" > 
                <input>
                    <input bind="txtID" />                    
                </input> 
                <output>
...
<gw:data id="datCheckPerID" onreceive="OnDataReceive(this)"> 
        <xml> 
            <dso  type="process" procedure="hr.sp_pro_check_per_id" > 
                <input>
...
<!------------------------------------>
<gw:data id="datPreCheckPerID" onreceive="OnDataReceive(this)"> 
        <xml> 
            <dso  type="process" procedure="hr.sp_pro_check_per_id" > 
                <input>
                    <input bind="txtRec_PK" />                    
...
<tr style="height:5%; width:100%">
  ...
  <td   style="border:0"><gw:imgBtn id="ibtnSearch"  alt="Popup"  img="tmp" text="Search"  onclick="OnSearch()" /></td>
...
<fieldset style="border-color:blue"><legend><font color="#e9361d" size="2" > Curriculum Vitae</font></legend>
    <table width='100%'>
        ...
        <td colspan=6><gw:list  id="lstEducation" value=" " maxlen = "10" styles='width:100%' onchange="ChangeShool()" > 
                            <data>
                                <%=CtlLib.SetListDataSQL("select code,CODE_NM from vhr_hr_code where id='HR0011' order by code_nm")%>||
                            </data>
                            </gw:list>
        </td>
        <td id="tr1" colspan=4 align=center style="white-space:nowrap">Graduation School</td>
        <td id="tr2" colspan=4 align=center style="white-space:nowrap;display:none"><a title="Click here to show Graduation School" onclick="OnShowPopup('HR0146',lstSchool)" href="#tips" >
            Graduation School
        </td>
        <td id="tr3" colspan=6 style="white-space:nowrap">  <gw:textbox id="txtSchool"  text="" maxlen = "100" styles='width:100%' /> </td>
        <td id="tr4" colspan=6 style="white-space:nowrap;display:none">  
            <gw:list  id="lstSchool" value=" " maxlen = "10" styles='width:100%' onchange="OnSetSchool()" > 
                            <data>
                                <%=CtlLib.SetListDataSQL("select code,CODE_NM from vhr_hr_code where id='HR0146' order by code_nm")%>||
                            </data>
             </gw:list>
        </td>
        <td colspan=4 align=center style="white-space:nowrap">Major</td>
        <td colspan=6>  <gw:textbox id="txtMajor"  text="" maxlen = "100" styles='width:100%' /> 
        ...
        <td colspan=6> <gw:textbox id="txtOtherLanguage"  text="" maxlen = "100" styles='width:100%' />  </td>
        <td colspan=4 align=center style="white-space:nowrap"></td>
        <td colspan=6>
        </td>
...
<fieldset> <legend><font color="#e9361d" size="1" >Apply</font></legend>
    <table width='100%'>
        ...
        <td colspan=6><gw:list  id="lstOrg_Code" value=" " maxlen = "100" styles='width:100%' >
                            <data>
                                <%=CtlLib.SetListDataSQL("SELECT PK, ORG_NM FROM COMM.TCO_ORG A WHERE a.DEL_IF=0 and nvl(a.end_date,to_char(sysdate+1,'yyyymmdd'))>to_char(sysdate,'yyyymmdd') ORDER BY NVL(SEQ,99999)")%>||
                            </data>
                        </gw:list>
        </td>
    ...
    <table width='100%'>
       ...
       <td colspan=4 align=center style="white-space:nowrap">Remark 3</td>
        ...
        <td colspan=3 align=center>
            <gw:datebox id="dtPass"  maxlen = "10" text="" styles='width:100%'lang="<%=Session("Lang")%>" nullaccept onchange="" />
        </td>
