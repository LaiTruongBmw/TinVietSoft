...
function BodyInit()
{
    ...
    if ( group_type == 1 )
    {
        data = "<%=CtlLib.SetListDataSQL("SELECT pk, specgrp_nm FROM tlg_it_specgrp WHERE del_if = 0 and ( grp_type = 1 or grp_type = 0 ) order by specgrp_nm ")%>";                 
    }
    else if ( group_type == 2 ) 
    {
        data = "<%=CtlLib.SetListDataSQL("SELECT pk, specgrp_nm FROM tlg_it_specgrp WHERE del_if = 0 and ( grp_type = 2 or grp_type = 0 ) order by specgrp_nm ")%>";             
    }
    else
    {
        data = "<%=CtlLib.SetListDataSQL("SELECT pk, specgrp_nm FROM tlg_it_specgrp WHERE del_if = 0 order by specgrp_nm ")%>";         
    }
...
<body>
    ...
    <gw:data id="pro_fpab00090" onreceive="OnDataReceive(this)"> 
        <xml> 
            <dso  type="list" procedure="st_lg_pro_fpab00090" > 
                <input>
    ...
    <gw:data id="data_fpab00090" onreceive="OnDataReceive(this)"> 
	    <xml> 
		    <dso type="grid"  function="st_lg_sel_fpab00090"  > 
			    <input bind="grdSpec" >
			        <input bind="txtSTItemPK" /> 
    ...
    <table style="width: 100%; height: 100%">
        <tr style="height: 1%">
            ...
            <td style="width: 1%">
                <gw:button id="btnSelect" img="select" alt="Select" onclick="OnSelect(grdSpec)" />
            </td>
            <td style="width: 1%" align="right">
                <gw:button id="btnCancel" img="cancel" alt="Cancel" onclick="OnCancel(grdSpec)" />
            </td>
        </tr>
        <tr style="height: 99%">
            <td colspan="7">
