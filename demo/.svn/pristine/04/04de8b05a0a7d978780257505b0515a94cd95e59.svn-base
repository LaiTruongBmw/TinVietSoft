...
function BodyInit()
{
    ...
    <%=CtlLib.SetGridColumnComboFormat("grid", 2,"SELECT A.CODE, A.CODE_NM FROM acnt.TAC_COMMCODE_DETAIL A, acnt.TAC_COMMCODE_MASTER B WHERE A.DEL_IF = 0 AND B.DEL_IF = 0  AND A.TAC_COMMCODE_MASTER_PK = B.PK AND B.ID = 'EACAB030'  ORDER BY A.CODE")%>;
...
function OnPopupSlipEntry()
{
	...
	if (rownum > 0)		 
	{
		if (event.col >= 1)
		{
			...
			var fpath   = System.RootURL + '/standard/forms/gf/hg/gfhg00020_popup.aspx?PK_ROW=' + pk + '&ROW=' + ctrl.row + '&ROWS=' + rows + '&APPRCANCEL=0' + "&ARRAY_PK=" + ls_arr;
			var wWidth  = 950;
			var wHeight = 600;
...
{
    var fpath   = System.RootURL + "/standard/forms/gf/co/gfco002005.aspx?comm_code="+ "" + "&comm_nm=" + '' + "&company=" + lstCompany.GetData() + '&dsqlid=ACNT.SP_SEL_USER_ID_NM&col_code=' + '' +"User ID" + '&col_nm=' + '' + "User name";
...
<body style="margin:0; padding:0;">
    ...
    <gw:data id="dso_sel_grd_dtl_slip_app_cancel" onreceive="SetTotal()">
        <xml>
            <dso type="grid" function="acnt.SP_SEL_GRD_SLIP_BY_APP_DATE">
                <input bind="grid">
    ...
    <gw:data id="dso_pro_sel_user"> 
        <xml> 
            <dso type="process" procedure="ACNT.SP_PRO_SEL_USER_ID_NM" > 
                <input> 
            ...
            <td width="100%">
                ...
                <tr>
                    ...
                    <td><gw:list id="lstVouchertype" onChange="OnChangeList()" value ="ALL" styles='width:100%'>
                        <data><%=CtlLib.SetListDataFUNC("SELECT SF_A_GET_COMMONCODE('EACAB030') FROM DUAL")%>|ALL|Select ALL</data>
                        </gw:list>
                    </td>
