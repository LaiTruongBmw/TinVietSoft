...
function BodyInit()
{
    ...
    /*if(idDsql != "ac_sel_60130120_pl_popup" && idDsql != "ACNT.SP_SEL_PL_POPUP_PLPK" && idDsql != "ACNT.SP_SEL_PL_POPUP_CENTERPK" )	 
    {
...
function Select()
{
	...
	else
	{
        ...
        for (i = 0; i<idGrid.GetGridControl().SelectedRows ; i++)
	    {
		    var row = idGrid.GetGridControl().SelectedRow(i);
          
            if ( row > 0 )
            {
                var code_data=new Array();
                //code
		        code_data[0] = idGrid.GetGridData( row, 0 );
		        //code name
		        code_data[1] = idGrid.GetGridData( row , 1 );
		        // value1 --> PK
		        code_data[2] = idGrid.GetGridData( row , 2 );
		        // value2
		        code_data[3] = idGrid.GetGridData( row, 3 );
		        // value3
		        code_data[4] = idGrid.GetGridData( row, 4 );
		        // value4
		        code_data[5] = idGrid.GetGridData( row , 5 );	
		        code_data[6] = idGrid.GetGridData( row , 6 );	
                //code_data[7] = idGrid.GetGridData( row, 7 );	
                arr_data[arr_data.length]= code_data;	           
            }		
	    }
