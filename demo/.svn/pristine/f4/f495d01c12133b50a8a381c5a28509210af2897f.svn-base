...
{ 
    ...
    { 
        var ls_data     = "<%=CtlLib.SetListDataSQL("SELECT TRIM(A.CODE), A.CODE_NM FROM TCO_ABCODE A, TCO_ABCODEGRP B WHERE A.DEL_IF = 0 AND B.DEL_IF = 0  AND A.TCO_ABCODEGRP_PK = B.PK AND B.ID = 'ACAB0070' AND A.USE_IF = 1  AND a.code = 'A' ORDER BY ORD")%>";
    }
...
<body>
    <gw:data id="dso_sel_trans">
        <xml>
            <dso type="grid" parameter="0,1,2,3,4" function="ac_sel_60130130_transgroup">
                <input bind="idGrid" >
    ...
    <gw:data id="dso_sel_trans_debit" onreceive="OnDataReceive(this)">
        <xml>
            <dso type="grid" parameter="0,1,2" function="ac_sel_60130130_account_drcr">
                <input bind="idGridDr" >
                    <input bind="txt_code"/>
                    <input bind="txt_name"/>
                    <input bind="txt_DRCR"/>
