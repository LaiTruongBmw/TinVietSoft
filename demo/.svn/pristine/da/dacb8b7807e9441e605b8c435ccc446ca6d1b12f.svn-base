...
function BodyInit()
{
	...
	lstCompany.SetDataText("<%=CtlLib.SetListDataSQL("select PK,PARTNER_NAME from COMM.TCO_COMPANY where DEL_IF = 0 AND ACTIVE_YN='Y'")%>");
...
<gw:data id="datDeptTree"   > 
        <xml> 
            <dso id="1" type="tree" function="acnt.sp_sel_plunit_tree_2" > 
                <input>
                    <input bind="lstCompany" /> 
                    <input bind="iduser_pk" />
                    <input bind="txtPLUnit" /> 
                </input> 
                <output bind="treDept" /> 
            </dso> 
        </xml> 
        ...
        <td>
            <table cellpadding="0" cellspacing="0" border="0" width="100%" >
                <tr>
                    ...
                    <td width="50%">
                         <gw:list id="lstCompany" styles='width:100%' onchange="OnChange_com()"></gw:list> 
                    </td>
                    <td width="15%" align="right"><gw:imgBtn id="btnSearch1" img="search"  alt="search" onclick="OnSearch()" /> </td>
                    <td width="15%" align="right"><gw:imgBtn id="btnSearch" img="select"  alt="Select" onclick="OnSelect()" /></td>
                </tr>
                <tr>
                    <td width="20%" align="left"><font color="black" ><b>Biz Unit&nbsp;</b></font></td>
                    <td width="50%"><gw:textbox id="txtPLUnit" onenterkey="OnSearch()" text="" styles="width:100%" /></td>
                </tr>
            </table>
        </td>
    </tr>
