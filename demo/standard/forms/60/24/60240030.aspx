...
function BuildHeader(ctrl){
	...
	if(first_day > 1){
		...
		obj.ColHidden(30+start_col) = false;
		obj.ColHidden(29+start_col) = false;
		obj.ColHidden(28+start_col) = false;
	}
	else
	{
		for(var i=0;i<=30;i++){
		...
		if(cur_month == 2){
			...
			obj.ColHidden(30+start_col) = true;
			obj.ColHidden(29+start_col) = true;
			obj.ColHidden(28+start_col) = true;
		}
		else if(cur_month == 4 || cur_month == 6 || cur_month == 11){
			obj.ColHidden(30+start_col) = true;
		}else{
			obj.ColHidden(30+start_col) = false;
			obj.ColHidden(29+start_col) = false;
...
function OnShowDetailBK()
{
    if(grdRFCUsing.row > 0)
    {
       ...
       var path = System.RootURL + "/standard/forms/60/24/60240030_popup1.aspx?d=" + select_date+"&type="+ type;
	   //alert(path)
...
function OnReport(obj)
 {
    if(obj=='1')
	{
		...
		{
			 var url = '/standard/reports/60/24/rpt_60240030_room_availability.aspx?p_date='+dtDate.value; 
	...
	if(obj=='3')
	{
		 var url = '/standard/reports/60/24/rpt_60240030_OCC_Forecast.aspx?p_date='+dtDate.value+'&p_days='+txtDays.text; 
		System.OpenTargetPage( System.RootURL+url , "newform" );
	}	
 }
...
<body>
    ...
    <gw:data id="data_htrm00010" onreceive="ResetHeader();"> 
        <xml>
           <dso  type="grid"   function="st_ht_sel_60240030" >
	...
	<gw:data id="data_htrm00010_2" onreceive="ResetHeader2();"> 
        <xml>
           <dso  type="grid"   function="st_ht_sel_60240030_note" >
