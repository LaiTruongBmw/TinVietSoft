 ...
 function OnClick(obj_type){
    switch(obj_type){
		...
		case "SETDATA":
			...
			{
				if(grdData.GetGridControl().IsSelected(i))
				{            
					if (!OnExits(grdData.GetGridData(i, G1_ID))){
						seqInit = OnMaxSeq();
						seqInit = parseInt(seqInit) + 1;
						
						grdResult.AddRow();
						lNewRow = grdResult.rows - 1;
						
						for ( j = 0 ; j <= G1_ITEM_GRP_PK ; j++)
						{
							if (j != G1_SEQ)
							{
								grdResult.SetGridText(lNewRow, j, grdData.GetGridData(i, j));
							} else {
								grdResult.SetGridText( lNewRow, G1_SEQ, seqInit);//sequence
							}
						}
					}
				}
		...
		case "GETDATA":
		   ...
		   {   
			  for(var i=1;i<grdResult.rows;i++)
			  {
				 ...
				 for(var j=0;j<grdResult.cols;j++)
				 {
					arrTemp[arrTemp.length]= grdResult.GetGridData(i,j);
				 }
...
function OnMaxSeq()
{
    ...
    else
    {
        for (var i = 1; i < grdResult.rows; i++)
        {
            if ( Number( grdResult.GetGridData(i,G1_SEQ) ) > maxSeq )
            {
                maxSeq = Number( grdResult.GetGridData(i,G1_SEQ) )
            }
        }
        return maxSeq;
    }
...
function OnExits(data){
	...
	for (var i = 1; i < grdResult.rows; i++)
	{
		if(grdResult.GetGridData(i,G1_ID) == data){
			flag = true;
		}
		
		if(flag) i = grdResult.rows;
	}
	return flag;
}
</script>
                        ...
                        <tr style="padding-bottom:2px" class="eco_bg">
                            <td align="left">
                                <table style="height:100%;">
                                    <tr>
                                        <td></td>
                                        <td style="width:200px"><gw:list id="lstCode" styles='width:100%' /></td>
                                    </tr>
                                </table>
                            </td>
                            <td>&nbsp;</td>
                        ...
                        <tr style="height:56%">
                            <td colspan="3" class="eco_line_t">
                                <gw:grid id='grdData' 
                                    ...
                                    sorting='T' styles='width:100%; height:200px'
                                    onclick="OnClick('DETAIL')"
                                    acceptnulldate='T' />
                            </td>
                        </tr>
						<tr style="padding-bottom:2px">
                            <td colspan="3" class="eco_line_t" align="right">
                                <table style="height:100%;">
                                    <tr>
										<td align="right"><gw:button id="btnRemove" img="delete" alt="Delete" text="Xóa bỏ" onclick="OnClick('REMOVE')" /></td>
										<td align="right"><gw:button id="btnGet" img="select" alt="Select" text="Lấy Data" onclick="OnClick('GETDATA')" /></td>
                                    </tr>
                                </table>
                            </td>
                        </tr>
