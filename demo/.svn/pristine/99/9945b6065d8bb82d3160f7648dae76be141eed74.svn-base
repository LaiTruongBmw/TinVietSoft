...
function BindingDataList()
{
    ...
    ls_data      = "<%=CtlLib.SetListDataSQL("select TRIM(CODE) code, A.code_nm FROM TAC_COMMCODE_DETAIL A, TAC_COMMCODE_MASTER B WHERE A.DEL_IF=0 and b.del_if = 0 and a.TAC_COMMCODE_MASTER_PK = b.pk AND TRIM(b.ID) = 'EACAB014' and a.code in ('ED', 'SO')")%>";        
	...
	var l_whnm = "<%=CtlLib.SetDataSQL("select wh_name from TLG_IN_WAREHOUSE where wh_id like 'VCSC CM' and del_if = 0")%>";    	
	txtWH_NM.text = l_whnm ;
	var l_whpk = "<%=CtlLib.SetDataSQL("select pk from TLG_IN_WAREHOUSE where wh_id like 'VCSC CM' and del_if = 0")%>";    	
...
//CHK|_MST_PK|_PK|_TCO_ITEM_PK|ITEM CODE|ITEM NAME|UNIT|QTY|PRICE|EXT PRICE|NET AMT|_COMPANY		
		...
		break;
		case "TO":
			var fpath   = System.RootURL + "/standard/forms/gf/ca/gfca00010_GetPO.aspx";
            var object  = System.OpenModal(fpath , 800 , 600 , 'resizable:yes;status:yes'); 
			var i, j;
			if(object != null)
			{
...
<gw:data id="dso_get_rate" onreceive="SetExRate()"> 
	...
	<gw:data id="grdMst" onreceive="OnDataReceive(this)"> 
		<xml> 
			<dso type="control" parameter="0,1,2,3,4,6,7,8,11,14,17,20,21,22,23,26,27,29,31,32,33,34,37,38,40,41" function="ac_sel_60110010_crca_do_cc" procedure="ac_upd_60110010_cancel_crca_do" > 				
				<inout>
    ...
    <gw:data id="dso_upd_crca_ctrlitem" onreceive="OnDataReceive(this)" >
        <xml>
            <dso type="grid" parameter="0,1,4,5,6,7,8,9" function="ac_sel_60110010_cancel" procedure="ac_upd_60110010_cancel">
                <input bind="grdInfor">
    ...
    <gw:data id="dso_upd_crca_item" onreceive="OnDataReceive(this)" >
        <xml>
            <dso type="grid" parameter="0,1,2,3,8,9,10,11,12,15,17,18,19,20,21" function="ac_sel_60110010_cancl_crcad_do" procedure="ac_upd_60110010_cancl_crcad_do" >
                <input bind="grdDtl">
                    <input bind="txtCRCA_PK"/>
                </input>
                <output bind="grdDtl"/>
    ...
    <gw:data id="dso_upd_crca_vat" onreceive="OnDataReceive(this)" >
        <xml>
            <dso type="grid" parameter="0,6,7,8,9,12,13,14" function="ac_sel_60110010_cancl_vat" procedure="ac_upd_60110010_cancl_vat" >
    ...
    <gw:data id="dso_upd_crca_pl">
        <xml>
            <dso type="grid" parameter="0,1" function="ac_sel_60110010_cancl_crcad_pl" procedure="ac_upd_60110010_cancl_crcad_pl" >
                <input bind="grdDtlPL">
                    <input bind="txtCRCA_PK"/>
                </input>
    ...
    <gw:data id="dso_upd_crca_cost" onreceive="OnDataReceive(this)"  >
        <xml>
            <dso type="grid" parameter="0,7" function="ac_sel_60110010_cancl_cost" procedure="ac_upd_60110010_cancl_cost" >
    ...
    <gw:data id="dso_ar_cancel" onreceive="OnDataReceive(this)" > 
        <xml> 
            <dso type="process" procedure="ac_pro_60110010_os_confcancel" >
                <input> 
					<input bind="txtcrca_pk_lst" />
                    <input bind="txtIndex" />			
    ...
    <gw:data id="dso_ar_copy" onreceive="OnDataReceive(this)" > 
        <xml> 
            <dso type="process" procedure="ac_pro_60110010_crca_copy" >
                <input> 
					<input bind="txtCRCA_PK" />                    			 
                </input>  
                <output>
