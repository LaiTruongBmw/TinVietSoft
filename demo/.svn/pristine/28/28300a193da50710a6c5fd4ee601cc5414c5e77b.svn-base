...
function BindingDataList()
{
    ...
    var ls_report   =  "<%=CtlLib.SetListDataSQL(" SELECT CODE, CODE_NM  FROM TCO_ABCODE A, TCO_ABCODEGRP B WHERE A.TCO_ABCODEGRP_PK = b.pk and b.id ='ACBG0150' and a.use_if = 1 and a.del_if = 0 and b.del_if = 0  and code like '1%' order by a.DEF_YN desc , code ")%>";
...
{
   var url = '/standard/reports/gf/ff/';
   var isPrint = 1;
...
function OnPopupDailySL()
{
        ...
        if (rownum > 0) 
        {
	        if ( ac_pk!='')
	        {
		        ...
		        var fpath = System.RootURL + '/standard/forms/gf/ka/gfka00100_popup.aspx?ac_pk='+ ac_pk + '&ac_nm=' + ac_nm + '&compk=' + lstCompany.GetData() + '&status=' + rbstatus.GetData() + '&lang='+ lstLanguage.GetData() + '&cur=' + lstCurrency.GetData() + '&date_fr=' + dtbMonth.GetData() + '&date_to=' + dtbMonth.GetData() + '&type_month=1';
		        var wWidth = 950;
		        var wHeight = 500;
...
<gw:data id="datGetRate" onreceive="onReceiveRate()" onerror="onDataErr(this)">
 ...
 <gw:data id="datExpenseInquiry" onreceive="onFormatGridData()" onerror="onDataErr(this)">
        <xml>
            <dso type="grid" function="ac_sel_601900140_expense_st">
                    ...
                    <td colspan="2">
                            <table border="0" cellpadding="0" cellspacing="0">
                                <tr>
                                       ...
                                       <td align="right"><gw:button id="idBtnPrint"  img="excel" text="Print"alt="Report"  onclick="onPrintReport()" /></td>
                                </tr>
                            </table> 
                    </td>
                </tr> 
