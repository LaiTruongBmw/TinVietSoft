...
function BodyInit()
{
	...
	txtTime.text="<%=request.QueryString("stime")%>";
	txtType.text="<%=request.QueryString("stype")%>";
	txtUpperDept_PK.text="<%=request.QueryString("upper_dept_pk")%>";
	datDailyEmpDetail.Call("SELECT");
}
...
<body>
<gw:data id="datDailyEmpDetail" onreceive="OnDataReceive()" > 
        <xml> 
            <dso  type="grid"  function="ST_HR_SEL_HREM00401_0"> 
	    ...
	    <table width="100%" height=7% cellspacing=0 cellpadding=0 >		
			<tr>
  			...
  			<tr>
				...
				<td align ="right" width=50%><b><gw:label id="lblRecord" text="" maxlen = "100" styles="color:#black;width:90%;font-weight: bold;font-size:12"></gw:label></b></td>
  			</tr>
		</table>
		<table width="100%" height=93% border=1 cellspacing=0 cellpadding=0 >		
			<tr>
				<td align = top height=100% >					
					<gw:grid   
                        id="grdEmployee"  
                        header="Department|Team/Section|Emp Id|Full Name|Join Date"
                        format="0|0|0|0|4"  
                        aligns="0|0|1|0|1"  
                        defaults="||||"  
                        editcol="0|0|0|0|0"  
                        widths="1500|1500|1200|2700|1600"  
...
<gw:textbox id="txtTime" styles="display:none"/>
