...
<script>
	...
	{         
	    ...
	    var data = "<%=CtlLib.SetListDataSQL("select a.pk, lpad('-',level ,'-')||a.grp_cd || ' - ' || a.grp_nm from tlg_it_itemgrp a where a.del_if = 0 and a.use_yn='Y' connect by prior a.pk = A.P_PK start with A.P_PK is null order siblings by grp_cd " ) %>||";     
    ...
    function OnReport(pos) 
	{
		...
		{
            case '1':
                var url = System.RootURL + "/standard/reports/ds/cd/rpt_epbp00140.aspx?p_date_type=" + idList.value + "&p_from_dt=" + dtFrom.value + "&p_to_dt=" + dtTo.value + "&p_slip_cust=" + url_encode(txtSlipCust.text) + "&p_item=" + url_encode(txtItem.text) + "&p_bal_yn=" + chkBal.value + "&p_req_bal=" + chkReqBal.value + "&p_type=" + lstType.value;
                System.OpenTargetPage(url);
                break;

            case '2':
                var url = System.RootURL + "/standard/reports/ds/cd/rpt_epbp00141.aspx?p_date_type=" + idList.value + "&p_from_dt=" + dtFrom.value + "&p_to_dt=" + dtTo.value + "&p_slip_cust=" + url_encode(txtSlipCust.text);
                System.OpenTargetPage(url);
                break;
        }
    }
...
<body>
    ...
    <gw:data id="data_epbp00140" onreceive="OnDataReceive(this)"> 
        <xml> 
            <dso id="1" type="grid" function="st_lg_sel_epbp00140" > 
                <input> 
