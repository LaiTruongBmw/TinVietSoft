...
function BindingDataList()
{
    ...
    var data = "<%=CtlLib.SetListDataSQL("SELECT pk, wh_id || '-' || wh_name wh_name FROM tlg_in_warehouse  WHERE del_if = 0 and use_yn = 'Y' and acc_yn = 'Y' ORDER BY wh_name ASC" )%>|ALL|ALL";    
...
function OnPopUp(iObj)
{
    ...
    case 'Account':
            ...
            var path = System.RootURL + "/standard/forms/60/03/60030010_popup_ifrs.aspx?comm_code=&comm_nm=&dsqlid=ac_sel_60170120_acc&val1=" + lstCompany.value + "&val2=" + lstWH.value + "&val3=" ;
...
{
    var url = System.RootURL + "/standard/reports/60/17/60170120.aspx?stdym_fr=" + dtFrom.value + "&stdym_to=" + dtTo.value + "&tco_company_pk=" + lstCompany.value ;
...
<body>
  ...
  <gw:data id="fmgf00120" onreceive="OnDataReceive(this)"> 
        <xml> 
            <dso id="1" type="grid"  function="ac_sel_60170120" > 
                <input bind="idGrid" >                 
    ...
    <!-------------------------------------------------------------------->
     <gw:data id="dso_inout_upd" onreceive="OnDataReceive(this)"> 
        <xml> 
            <dso  type="process" procedure="ac_pro_60170120" > 
                <input>
                   <input bind="lstCompany" />
                   <input bind="dtFrom"/>
                   <input bind="dtTo"/>
    ...
    <!-------------------------------------------------------------------->    
     <gw:data id="dso_search_seq" onreceive="OnDataReceive(this)"> 
        <xml> 
            <dso type="process" procedure="ac_PRO_60170120_1" > 
                <input>
                   <input bind="lstCompany" />
                   <input bind="dtFrom"/>
                   <input bind="dtTo"/>
	...
	<!-------------------------------------------------------------------->   	
     <gw:data id="dso_cancel_transfer" onreceive="OnDataReceive(this)"> 
        <xml> 
            <dso  type="process" procedure="ac_pro_60170120_2" > 
                <input>
                   <input bind="lstCompany" />
                   <input bind="dtFrom"/>
                   <input bind="dtTo"/>
	...
	<!-------------------------------------------------------------------->   
     <gw:data id="dso_make_slip" onreceive="OnDataReceive(this)"> 
        <xml> 
            <dso  type="process" procedure="ac_PRO_60170120_Make_Slip" > 
                <input>
                   <input bind="lstCompany" />
                   <input bind="dtFrom"/>
                   <input bind="dtTo"/>
	...
	<!-------------------------------------------------------------------->   
     <gw:data id="dso_cancel_slip" onreceive="OnDataReceive(this)"> 
        <xml> 
            <dso  type="process" procedure="ac_PRO_60170120_CANCEL_SLIP" > 
                <input>
                   <input bind="txtSeq" />
                 </input>
                <output>
