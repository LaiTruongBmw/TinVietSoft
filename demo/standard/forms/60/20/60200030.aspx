...
function BodyInit()
{
    ...
    var ls_lang     = "<%=CtlLib.SetListDataFUNC("SELECT ac_A_GET_COMMONCODE('COAB0070') FROM DUAL")%>";
    ...
    var data = "<%=CtlLib.SetListDataFUNC("SELECT ac_A_GET_DEFAULT('ACBG0040') FROM DUAL")%>";
    arr  = data.split("|");
...
function onReport()
{
   if(lstLang.value=="VIE")
	{
		var url = System.RootURL + '/standard/reports/60/26/rpt_602630_yearly_fc_vie.aspx?p_year='+dtYear.value+'&p_tco_company_pk='+lstCompany.value+'&p_scale='+txtScale.text+'&p_ccy='+lstCurrent.value+'&p_exrate='+txtrate1.text+'&p_status='+rbstatus.value+'&p_language='+lstLang.value;
		System.OpenTargetPage(url); 
	}
	else
	{
		var url = System.RootURL + '/standard/reports/60/26/rpt_602630_yearly_fc_eng_vie_kor.aspx?p_year='+dtYear.value+'&p_tco_company_pk='+lstCompany.value+'&p_scale='+txtScale.text+'&p_ccy='+lstCurrent.value+'&p_exrate='+txtrate1.text+'&p_status='+rbstatus.value+'&p_language='+lstLang.value;
		System.OpenTargetPage(url); 
	}
}
...
{   
    ...
    {
        case "ENG-KOR":
            grid_cf_di.GetGridControl().ColHidden(G_Item)          = false;
            grid_cf_di.GetGridControl().ColHidden(G_Item_Local)    = true;
            grid_cf_di.GetGridControl().ColHidden(G_Item_Korean)   = false;
            grid_cf_di.GetGridControl().Cell(0, 0, G_Item, 0, G_Item) = "Item";
            grid_cf_di.GetGridControl().Cell(0, 0, G_Item_Korean, 0, G_Item_Korean) = "Item Korean";
        break;
        case "VIE":
            grid_cf_di.GetGridControl().ColHidden(G_Item)          = true;
            grid_cf_di.GetGridControl().ColHidden(G_Item_Local)    = false;
            grid_cf_di.GetGridControl().ColHidden(G_Item_Korean)   = true;
            grid_cf_di.GetGridControl().Cell(0, 0, G_Item_Local, 0, G_Item_Local) = "Item Local";
        break;
        case "ENG":
            grid_cf_di.GetGridControl().ColHidden(G_Item)          = false;
            grid_cf_di.GetGridControl().ColHidden(G_Item_Local)    = true;
    ...
    grid_cf_di.GetGridControl().Cell(0, 0, G_Jan, 0, G_Jan)   = "Jan - " + dtYear.GetData();
    grid_cf_di.GetGridControl().Cell(0, 0, G_Feb, 0, G_Feb)   = "Feb - " + dtYear.GetData();
    grid_cf_di.GetGridControl().Cell(0, 0, G_Mar, 0, G_Mar)   = "Mar - " + dtYear.GetData();
    grid_cf_di.GetGridControl().Cell(0, 0, G_Apr, 0, G_Apr)   = "Apr - " + dtYear.GetData();
    grid_cf_di.GetGridControl().Cell(0, 0, G_May, 0, G_May)   = "May - " + dtYear.GetData();
    grid_cf_di.GetGridControl().Cell(0, 0, G_June, 0, G_June) = "June - " + dtYear.GetData();
    grid_cf_di.GetGridControl().Cell(0, 0, G_July, 0, G_July) = "July - " + dtYear.GetData();
    grid_cf_di.GetGridControl().Cell(0, 0, G_Aug, 0, G_Aug) = "Aug - " + dtYear.GetData();
    grid_cf_di.GetGridControl().Cell(0, 0, G_Sep, 0, G_Sep) = "Sep - " + dtYear.GetData();
    grid_cf_di.GetGridControl().Cell(0, 0, G_Oct, 0, G_Oct) = "Oct - " + dtYear.GetData();
    grid_cf_di.GetGridControl().Cell(0, 0, G_Nov, 0, G_Nov) = "Nov - " + dtYear.GetData();
    grid_cf_di.GetGridControl().Cell(0, 0, G_Dec, 0, G_Dec) = "Dec - " + dtYear.GetData();
}
...
function onSetFont(p_oData)
{
    switch(p_oData.id)
    {
        case "dso_sel_grd_mc":
            ...
            for (i = 1; i < ctrl.Rows; i++)
	        {   
	            if(grid_cf_di.GetGridData(i,G_Font_Stype)== "B")
                {
	                ctrl.Cell(14, i, G_Item, i, G_Dec) = false;
	                grid_cf_di.SetCellBold(i, G_Item, i, G_Dec, true);
                }
                else if (grid_cf_di.GetGridData(i,G_Font_Stype)== "I")
                {
	                grid_cf_di.SetCellBold(i, G_Item, i, G_Dec, false);
	            ...
	            //set color
	            grid_cf_di.SetCellBgColor(i, G_Item, i, G_Dec, Number(grid_cf_di.GetGridData(i, G_Prn_Color)));
	        }
...
<!------------------------------------------------------------------------>
 <gw:data id="dso_getCompany" onreceive=""> 
    ...
    <gw:data id="dso_sel_grd_mc" onreceive="onSetFont(this)" onerror="OnDataError(this)">
        <xml>
            <dso type="grid" function="ac_sel_60200030_mc">
