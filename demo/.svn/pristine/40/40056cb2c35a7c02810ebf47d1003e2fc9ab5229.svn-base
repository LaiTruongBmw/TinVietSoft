    ...
    if (grdAttach.rows > 0)
     {
        ...
        if(filepk!='')
        {
			var url =System.RootURL + "/standard/forms/fp/ab/fpab00720_DownFile.aspx?img_pk=" + filepk + "&table_name=" + txtTableName.text;
...
<body>
    ...
    <gw:data id="data_sql" onreceive="OnDataReceive(this)"> 
    	<xml> 
	        <dso  id="1" type="grid" parameter="0,4,5" function="st_lg_sel_dsbs00218"  procedure="st_lg_upd_dsbs00218">
	            <input>
	...
	<table id='id_4' style="width:100%;height:100%" cellpadding='0' cellspacing='0'>
		<tr style="width:100%; height:1%">
			...
			<td width="1%" valign='top'>
				<gw:button id="btS" img="attach" alt="Attach file"  onclick="AttachFiles()" styles='display:none'/>
			</td>
			<td width="1%" valign='top'>
				<gw:button id="btRefresh" img="refresh" alt="Refresh List"  onclick="OnRefresh()" />
			</td>
			<td width="1%" valign='top'>
				<gw:button id="btSaveAttact" img="save" alt="Update"  onclick="OnSave('Attach_file')" styles='display:none'/>
			</td>
			<td width="1%">
				<gw:button id="btDeletedtl" img="delete" alt="Delete"  onclick="OnDelete('Attach_file')" styles='display:none'/>
			</td>
			<td width="1%">
...
<!---------------------------------------------------------------------->
<gw:image id="imgFile" table_name="<%=l_user %>TLG_SA_SALEORDER_FILES" procedure="<%=l_user %>lg_upd_dsbs00218_files" view="/binary/ViewFile.aspx"
...
<gw:textbox id="txtTableName" text="TLG_SA_SALEORDER_FILES" maxlen="100" styles='width:100%;display:none' />
