...
switch(lang)
    ...
    case "ENG":
                ...
                switch (lstREPORT.value)
                {
					...
					break;
					case 'PSY':	
						url = '/standard/reports/60/08/rpt_60080210_eng.aspx?company_pk=' + lstCOMPANY.GetData() + '&dtYear=' + dtYear.GetData()+ '&account_pk=' + txtAC_PK.text + '&cust_pk=' + txtTCO_BUSPARTNER_PK.text + '&ccy=' + lstCCY.GetData()+ '&status=' + rdoSTATUS.GetData()+'&bookccy='+lstBookCcy.value+'&partner_type='+lstPartnerType.value;
						System.OpenTargetPage( System.RootURL+url , 'newform' );
					break;
                }
                System.OpenTargetPage( System.RootURL+url , 'newform' );
  ...
  <body>
	...
	<gw:data id="dso_search" onreceive="OnDataReceive(this)">
        <xml>
            <dso type="grid" function="ac_sel_60080210">
