...
function OnFormatGrid()
{
    ...
    data = "<%=CtlLib.SetListDataSQLPara("select a.pk, lpad('-',level ,'-')||a.grp_cd || ' - ' || a.grp_nm from tlg_it_itemgrp a where a.del_if = 0 and a.use_yn='Y' and a.group_type is not null and ( SALE_YN ={0} or PUR_YN ={1} or PROD_YN ={2} or SEMI_PROD_YN ={3} or MAT_YN ={4} or SUB_MAT_YN = {5} ) connect by prior a.pk = A.P_PK start with A.P_PK is null order siblings by grp_cd ", Request.querystring("group_type")) %>";     
	...
	var p_req_date   = "<%=Request.querystring("p_req_date")%>";
	
	txtItem.text  = p_item_code;
	lstDept.value = p_po_dept_pk;
	dtDate.value  = p_req_date;
...
<body>     
    <!---------------------------------------------------------------->
    <gw:data id="data_fpab00075_1" onreceive="OnReceiveData(this)"> 
		<xml> 
           <dso id="1" type="grid" function="st_lg_sel_fpab00075_1" > 
                  <input bind="idGrid" >
				  	<input bind="lstDept" />
				  	<input bind="lstGroup" />                     
                    <input bind="txtItem" />
	...
	<!---------------------------------------------------------------->
    <gw:data id="pro_fpab00075" onreceive="OnReceiveData(this)">
        <xml>
            <dso  type="list" procedure="st_lg_pro_fpab00075" >
                <input>
    ...
    <table style="width: 100%; height: 100%">
        <tr style="height: 1%">
			...
			<td style="width: 10%"   >
                <gw:datebox id="dtDate" lang="1" width="100%" />
            </td>					
        </tr> 
        <tr style="height: 1%">
            <td style="width: 5%" align="right">
        ...
        <tr style="height: 48%">
            <td colspan="8">
				...
				<gw:grid id='idGrid' 
					...
					aligns='0|0|0|1|0|1|0|0|0|1|0|0' 
					check='|||||||||||' 
        ...
        <tr style="height: 49%">
            <td colspan="8">                 
				<gw:grid id='idGrid2' 
					...
					aligns='0|0|0|1|0|1|3|0|0|0|1|0|0' 
					check='||||||||||||' 
