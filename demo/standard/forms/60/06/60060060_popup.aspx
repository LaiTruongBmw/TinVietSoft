...
var Ispopup='';
var acc_pk, ac_cd, ac_nm,company_pk,from_dt, to_dt,status;
function BodyInit()
{
    ...
    ac_cd              = "<%=Request.querystring("ac_cd")%>";
    ac_nm              = "<%=Request.querystring("ac_nm")%>";
    company_pk         = "<%=Request.querystring("company_pk")%>";
    from_dt            = "<%=Request.querystring("from_date")%>";
    to_dt              = "<%=Request.querystring("to_date")%>";
    status             = "<%=Request.querystring("status")%>";
       
    txtAccPK.SetDataText(acc_pk);
    txtAccCode.SetDataText(ac_cd);
    txtAccName.SetDataText(ac_nm);
    lstCompany.SetDataText(company_pk);
    dtbFrom.SetDataText(from_dt);
    dtbTo.SetDataText(to_dt);
    rdoSlipStatus.SetDataText(status);
    onSearch();
...
if(Trim(txtAccPK.text)=="")
   ...
   {
            var url =System.RootURL + '/standard/reports/gf/ka/gfka00020_DailySL.aspx'+rqt;
            System.OpenTargetPage( url , 'newform' );
            return;
   }
   if (lstReportType.value=="AS")
   {
        var url =System.RootURL + '/standard/reports/gf/ka/gfka00020_DailySL_CCY.aspx'+rqt;
        System.OpenTargetPage( url , 'newform' );
        return;
   }
 */
...
<body>
   <gw:data id="datSLSum" onreceive="OnDataReceive(this)" >
        <xml>
            <dso type="grid"  function="ac_sel_daily_sl_sum">
                <input bind="grdSLSum">
   ...
   <gw:data id="datSLByDate" onreceive="OnDataReceive(this)" >
        <xml>
            <dso type="grid"  function="ac_sel_daily_sl_date">
      ...
      <gw:data id="datSLByVoucher" onreceive="OnDataReceive(this)" >
        <xml>
            <dso type="grid"  function="ac_sel_daily_sl_voucher">
