...
public partial class rpt_hrti00800_5 : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        string p_user, p_tco_org_pk, p_wg, p_year;
        string p_input, p_search_ot, p_ot, p_nation,p_status,p_lang;
		...
		if(p_lang == "ENG")
		{
			TemplateFile = "rpt_hrti00800_5_eng.xls";
		}
        else
			TemplateFile = "rpt_hrti00800_5_vie.xls";

        string TempFile = "../../../../system/temp/rpt_hrti00800_5_" + Session["User_ID"].ToString() + ".xls";
		...
		string para = "'" + p_tco_org_pk + "','" + p_input + "','" + p_search_ot + "','" + p_nation + "','" + p_status + "','" + p_wg + "','" + p_ot
							+ "','" + p_year + "','" + p_lang +"'";
        


        //----------------------------------
	DataTable dt_emp = CtlLib.TableReadOpenCursor("ST_HR_RPT_HRTI00800_5", para);
		...
		for (int l_addrow = 1; l_addrow < dt_emp.Rows.Count; l_addrow++)
        {
			...
			if(p_year != "" )
			{
				if (p_lang =="ENG")
				{
					exSheet.Cells["A1"].Value = "OVERTIME SUMMARY IN YEAR " + dt_emp.Rows[0][0].ToString();
					exSheet.Cells["A2"].Value = "Report Date "+ dt_emp.Rows[0][1].ToString();
				}	
				else
				{
					exSheet.Cells["A1"].Value = "TỔNG HỢP TĂNG CA TRONG NĂM " + dt_emp.Rows[0][0].ToString();
					exSheet.Cells["A2"].Value = "Ngày Báo Cáo "+ dt_emp.Rows[0][1].ToString();
				}	
			}
