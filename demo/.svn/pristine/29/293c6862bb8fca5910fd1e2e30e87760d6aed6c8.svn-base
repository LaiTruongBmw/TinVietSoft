		    ...
		    <table style="width:100%;height:100%" border="0" cellpadding="0" cellspacing="0">
                ...
                <tr height="7%" style="width:100%;"  valign="top"> 
			        ...
			        <td align="center" valign="middle" style="border-left:0;border-right:0"><gw:list  id="lstMaster_Code_1" value='ALL' maxlen = "100" styles='width:80%' onchange="OnChangeMaster(1)">
                                        <data><%= CtlLib.SetListDataSQL("select a.pk, a.app_no from thr_rec_request a where a.del_if=0 and a.app_status ='02' ")%>|ALL|Select All
                                        </data>
                                        </gw:list></td>
			        <td align="center" valign="middle" style="border-left:0;border-right:0"><gw:list  id="lstDetail_Code_1" value='ALL' maxlen = "100" styles='width:80%' onchange="">
                                        <data><%= CtlLib.SetListDataSQL("SELECT to_char(B.PK) t1,A.APP_NO||' - '|| (SELECT V.CODE_NM FROM VHR_HR_CODE V WHERE V.ID='HR0008' AND V.CODE=B.POS_TYPE ) t2  FROM THR_REC_REQUEST A, THR_REC_REQUEST_DETAIL B WHERE A.DEL_IF = 0 And B.DEL_IF = 0 And A.PK = B.THR_REC_REQUEST_PK and nvl(b.app_status,'01')='03' ")%>|ALL|Select All
                                        </data>
                                        </gw:list></td>
