...
function BodyInit()
{
    ...
    var ls_data1    = "<%=CtlLib.SetListDataSQL("SELECT A.PK, A.org_NM FROM TCO_org A WHERE del_if=0 ")%>";
...
function OnPre()
{
    ...
    if(callerWindowObj.name == "60080040_popup" || callerWindowObj.name == "60080090" || callerWindowObj.name == "60080040" || callerWindowObj.name == "60060070_0" || callerWindowObj.name == "60080070" ) 
    {
...
{
	var path = System.RootURL + '/standard/forms/gf/hg/gfhg00020_1.aspx?com_pk='+ lstCompany.GetData() + '&seq=' + txtseq.GetData() + '&status=' + txtStatus.GetData() + '&userid=' + txtProposedByUserID.GetData() + '&usernm=' + txtProposedByUserName.GetData() + '&userpk=' + txtProposedByUserPk.GetData() + '&dsqlid=ACNT.SP_PRO_CTL_MST_CONFIRM_CANCEL' + '&table=TAC_HGTRH';
...
<gw:data id="dso_getCompany" onreceive="OnDataReceive(this)"> 
 ...
 <gw:data id="dso_udp_ctl_mst_slip_entry" onreceive="OnDataReceive(this)"> 
        <xml> 
            <dso type="control" function="ac_sel_60080040_ctl_mst_slip">
            <inout> 
                <inout bind="txtseq"/>
    ...
    <gw:data id="dso_sel_grd_dtl_slip_entry_dr" onreceive="OnDataReceive(this)">
        <xml>
            <dso type="grid" function="ac_sel_60080040_grd_dtl_slip">
                <input bind="grdDebit" >
                    <input bind="txtseq"/>
    ...
    <gw:data id="dso_sel_count_ctl_item" onreceive="OnDataReceive(this)"> 
        <xml> 
            <dso type="process" procedure="ac_pro_60080040_dtl_cnt_slip" > 
                <input> 
                     <input bind="txtacpk"/>
                     <input bind="txt_drcr_type"/>
                     <input bind="txthgtrd"/>
    ...
    <gw:data id="dso_sel_control_new_item" onreceive="ItemStyle()">
        <xml>
            <dso type="grid" function="ac_sel_6008004_ctl_item_slip" >
    ...
    <gw:data id="dso_upd_approval_cancel" onreceive="SetButtonEnable()"> 
        <xml> 
            <dso type="process" procedure="ac_pro_60080040_app_cancel" > 
                <input> 
