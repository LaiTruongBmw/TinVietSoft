...
function BodyInit()
{
    ...
    var ls_data1    = "<%=CtlLib.SetListDataSQL("SELECT A.PK, A.DEPT_NM FROM TCO_DEPT A WHERE A.DEPT_TYPE = '06' AND A.TCO_COMPANY_PK = 1 AND (A.END_DATE >= '20080101' OR A.END_DATE IS NULL)")%>";
...
<!------------------------------------------------------------------------------------------------------->
 ...
 <gw:data id="dso_udp_ctl_mst_slip_entry" onreceive="OnDataReceive(this)"> 
    ...
    <gw:data id="dso_sel_grd_dtl_slip_entry_dr" onreceive="OnDataReceive(this)">
        <xml>
            <dso type="grid" function="ac_sel_60090040_tacc_popup">
                <input bind="grdDebit" >
                    <input bind="txtseq"/>
    ...
    <gw:data id="dso_sel_count_ctl_item" onreceive="OnDataReceive(this)"> 
        <xml> 
            <dso type="process" procedure="ac_sel_60090040_2_tacc_popup" > 
                <input> 
                     <input bind="txtacpk"/>
                     <input bind="txt_drcr_type"/>
                     <input bind="txthgtrd"/>
    ...
    <gw:data id="dso_sel_control_new_item" onreceive="ItemStyle()">
        <xml>
            <dso type="grid" function="ac_sel_60090040_3_tacc_popup" >
    ...
    <gw:data id="dso_upd_approval_cancel" onreceive="SetButtonEnable()"> 
        <xml> 
            <dso type="process" procedure="ac_sel_60090040_4_tacc_popup" > 
                <input> 
