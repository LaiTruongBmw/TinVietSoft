...
function BodyInit()
{
	...
	data = "<%=CtlLib.SetListDataFUNC("SELECT ST_HR_GET_HRCODE2('HR0222', '" + Session("SESSION_LANG").ToString() + "') FROM DUAL" )%>";    
	lstKind.SetDataText(data);
	lstKind.value ="";
	
//	data = "<%=CtlLib.SetListDataFUNC("SELECT ST_HR_GET_HRCODE2('HR0149', '" + Session("SESSION_LANG").ToString() + "') FROM DUAL" )%>";    
//	lstTreatKind.SetDataText(data);
//	lstTreatKind.value ="";
	
	var t1 = "<%=CtlLib.SetGridColumnDataSQL("select code,decode('" + Session("SESSION_LANG").ToString() + "', 'ENG', a.code_nm, a.code_fnm) from vhr_hr_code a where id='HR0222' order by code_nm")%>";
    grdInsert.SetComboFormat(G1_KIND,t1);
	
    t1 = "<%=CtlLib.SetGridColumnDataSQL("select code,decode('" + Session("SESSION_LANG").ToString() + "', 'ENG', a.code_nm, a.code_fnm) from vhr_hr_code a where id='HR0149' union all select null,null from dual order by 1")%>";
    grdInsert.SetComboFormat(G1_TREAT_KIND,t1);
...
{ 
    ...
    {
        case 1:
			data_hrem00701_1.Call("SELECT");
        break;
		case 2:
			dattreatkind.Call("SELECT");
		break;
    }
...
function OnValidate()
{
	...
	{
		if(grdInsert.GetGridData(i, G1_KIND) =="")
		{
		...
		{
			alert((v_language =="ENG")?"You have input start date at row " + i :"Bạn phải nhập cho ngày bắt đầu tại dòng " + i);
			return false;
		} 
		if(grdInsert.GetGridData(i, G1_TREAT_KIND) =="")
		{
			alert((v_language =="ENG")?"You have input to treat kind at row " + i :"Bạn phải nhập cho đến ngày kết thúc tại dòng " + i);
			return false;
		}  
	}
	return true;
...
function OnSelect(idGrid)
{
	for (i = 1; i< idGrid.rows  ; i++)
	{
		...
		if (idGrid.GetGridData(i, 0) == "-1" && (!CheckDataExist( grdInsert, col_val)))
		{
			...
			if(lstKind.value !="")
				grdInsert.SetGridText(grdInsert.rows-1, G1_KIND, lstKind.value);
			
			if(lstTreatKind.value != "")
				grdInsert.SetGridText(grdInsert.rows-1, G1_TREAT_KIND, lstTreatKind.value);
...
{	
	    ...
	    for ( var i =  0 ; i < ctrl.SelectedRows ; i++ )
		    {
			    ...
			    {		
					if(lstKind.value !="")
						grdInsert.SetGridText(row, G1_KIND, lstKind.value);
					
					if(lstTreatKind.value != "")
						grdInsert.SetGridText(row, G1_TREAT_KIND, lstTreatKind.value);
					...
					if(txtReason.text != "")
						grdInsert.SetGridText(row, G1_REASON, txtReason.text);
	...
	else
	{
		...
		{
			if(lstKind.value !="")
				grdInsert.SetGridText(i, G1_KIND, lstKind.value);
			
			if(lstTreatKind.value != "")
				grdInsert.SetGridText(i, G1_TREAT_KIND, lstTreatKind.value);
			...
			if(txtReason.text != "")
				grdInsert.SetGridText(i, G1_REASON, txtReason.text);
...
<body>
    ...
    <!---------------------------------------------------------------->
    <gw:data id="data_hrem00701_1" onreceive="OnReceiveData(this)"> 
    <xml> 
           <dso id="5" type="grid" function="ST_HR_SEL_HREM00701_1" > 
...
<gw:data id="data_hrem00701_0" onreceive="OnReceiveData(this)" > 
        <xml> 
            <dso  type="grid"  parameter="1,5,6,7,8,9,10,11,12,13" function="ST_HR_SEL_HREM00701_0" procedure="ST_HR_UPD_HREM00701_0"> 
...
<gw:data id="dattreatkind" onreceive="OnDataReceive(this)">
	<xml>
		<dso type="list" procedure="ST_HR_SEL_HREM00701_2">
			<input>
				<input bind="lstKind"/>
				<input bind="txtlanguage"/>
			</input>
			<output>
				<output bind="lstTreatKind"/>
			</output>
		</dso>
	</xml>
</gw:data>
						...
						<table style="background:#D6EAF8;width: 100%; height: 100%" >
							<tr>
								...
								<td colspan=20 style="width: 20%">
									<gw:list id="lstKind" styles="width:100%" onchange="OnSearch(2)" />
								</td>
								<td colspan=15 style="width: 15%">
									From Date
								</td>
								...
								<td colspan=15 style="width: 15%">
									<gw:textbox id="txtReason" styles="width:100%" onenterkey="" />
								</td>
							</tr>
							<tr>
								<td colspan=15 style="width: 15%">
								...
								<td colspan=20 style="width: 20%">
									<gw:list id="lstTreatKind" styles="width:100%" onchange="OnSearch()" />
								</td>
								<td colspan=15 style="width: 15%">
									To Date
								</td>
        ...
        <tr style="height: 55%">
            <td colspan="100">
                <gw:grid id="grdInsert" 
				    header="No|_EMP_PK|Organization|Employee ID|Employee Name|Kind(*)|Start Date(*)| End Date(*)|Treat Kind(*)|Amount|Reason|Times|Description|_SEQ"
                    format="0|0|0|0|0|0|4|4|0|2|0|2|0|0" 
