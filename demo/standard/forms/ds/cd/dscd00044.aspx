...
<script>
    ...
    function BodyInit() 
	{
		...
		//--------------------
		grdDetail.GetGridControl().ColFormat(G_REQ_QTY) = "###,###,###,###,###.##";
		//--------------------
        data_dscd00044.Call('SELECT');
    }
        ...
        switch (obj.id) 
		{
            case 'data_dscd00044':
                if (grdDetail.rows > 1) 
				{
                    ...
                    grdDetail.SetCellBgColor(1, G_REQ_QTY, grdDetail.rows - 1, G_REQ_QTY, 0xCCFFFF);
                }
            break;
        }
    }
...
<body>
	<!------------------------------------------------------------------>
    <gw:data id="data_dscd00044" onreceive="OnDataReceive(this)"> 
        <xml> 
            <dso id="1" type="grid"  function="st_lg_sel_dscd00044" > 
                <input> 
    ...
    <table style="width: 100%; height: 100%">
        ...
        <tr style="height: 100%">
            <td colspan="2">
                <gw:grid id='grdDetail' header='Date|Slip No|_TR_ITEM_PK|Item Code|Item Name|UOM|Req Qty'
