...
{
	var data="";
	data = "<%=CtlLib.SetGridColumnDataSQL("SELECT code,NAME FROM  TCO_COMMCODE WHERE DEL_IF=0 AND PARENT_CODE='PTYPE'")%>";
...
function OnReport(obj)
{
	...
	if(obj=='All')
	{
		if(grdMST.rows >1)
		{
			var url =System.RootURL + '/standard/reports/60/28/rpt_60280110_Deposit_List.aspx?p_dtfrom='+dtFrom.value+'&p_dtto='+dtTo.value+'&p_room_no='+txtRoom.text;
			System.OpenTargetPage(url);  
		}
	}
}
...
<gw:data id="dso_htfo00470" onreceive="OnDataReceive(this)">
	<xml> 
		<dso type="grid"  function="st_ht_sel_60280110_advance_list">   
			<input bind="grdMST">
				<input bind="dtFrom" />
				<input bind="dtTo" />
				<input bind="txtRoom" />
