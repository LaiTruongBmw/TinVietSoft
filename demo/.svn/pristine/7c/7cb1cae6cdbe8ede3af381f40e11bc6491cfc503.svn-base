...
function OnReport(obj)
 {
	if(obj=='1')
	{
		 ...
		 {
			var url = '/standard/reports/ho/28/rpt_60280080.aspx?p_from='+dtFromDate.GetData()+"&p_to="+dtToDate.GetData() + "&p_invoice=" + txtInvoiceNo.GetData() + "&p_roomno="  + txtRoomNo.GetData() + "&p_loc=POINT-05&p_pay=" + lstPayment.GetData(); 
	...
	if(obj=='2')
	{
		 if(grdMinibarReport.rows > 1)
		 {
			var url = '/standard/reports/ho/28/rpt_60280080_rvn_detail.aspx?p_dtfrom='+dtFromDate.GetData()+"&p_dtto="+dtToDate.GetData(); 
			System.OpenTargetPage( System.RootURL+url , "newform" );
		 }
	}
