...
{   
     ...
     lstSearchDept.value="";
     //--------ASSET TYPE     
     data = "<%=CtlLib.SetListDataSQL("select pk, asset_type_code || ' * '| | asset_type_name from tlg_ma_asset_type a where  del_if=0 order by asset_type_code")%>";       
	 lstSearchAssType.SetDataText(data+'||');
     lstSearchAssType.value='';
     //--------ASSET GROUP 	 	 	 		 	 
	 data = "<%=CtlLib.SetListDataSQL("select pk, grp_code || ' * '| | grp_name from tlg_ma_asset_group a where  del_if=0 order by grp_code")%>";       
	 lstSearchAssGrp.SetDataText(data+'||');
     lstSearchAssGrp.value=''; 

	 idGrid.GetGridControl().ColFormat(G_MA_QTY)        = "###,###,###.##";
	 idGrid.GetGridControl().ColFormat(G_ORG_AMOUNT)    = "###,###,###.##"; 
	 idGrid.GetGridControl().ColFormat(G_REMAIN_AMOUNT) = "###,###,###.##"; 
	 	 
	 idGrid2.GetGridControl().ColFormat(G_MA_QTY)        = "###,###,###.##";
	 idGrid2.GetGridControl().ColFormat(G_ORG_AMOUNT)    = "###,###,###.##"; 
	 idGrid2.GetGridControl().ColFormat(G_REMAIN_AMOUNT) = "###,###,###.##";  
}
//-----------------------------------------------------------------------------------------------
...
function OnRemove()
{
  if(idGrid2.row > 0)
  {
    var col_val = idGrid2.GetGridData( idGrid2.row, G_ASSET_PK);
...
function SetRowBackGroundColor(p_oGrid, p_col_val, p_color)
{
   ...
   {
      if(p_oGrid.GetGridData( i, G_ASSET_PK) == p_col_val)
      {
...
function CheckDataExist(p_oGrid,p_value)
{
    if ( chkDuplicate.value == 'Y' )
    {
       ...
       for(var i=1;i<p_oGrid.rows;i++)
       {
              if(p_oGrid.GetGridData(i, G_ASSET_PK) == p_value)
              {
                 return true;
              }
       }
...
function OnGridCellDoubleClick(oGrid)
{
      ...
      {
            var col_val = oGrid.GetGridData(event.row, G_ASSET_PK );
...
function OnAdd(oGrid)
{
	for (i = 0; i<oGrid.GetGridControl().SelectedRows ; i++)
	{
        ...
        var col_val = oGrid.GetGridData( row, G_ASSET_PK);		
...
<body>
    ...
    <gw:data id="data_fpab00590" onreceive="OnReceiveData(this)"> 
    <xml> 
           <dso id="1" type="grid" function="st_lg_sel_fpab00590" > 
                  <input bind="idGrid" >
    ...
    <table style="width: 100%; height: 100%" border="0">
        ...
        <tr style="height: 20%">
            <td colspan="10">
                <gw:grid id='idGrid' header='_pk|Asset Code|Asset Name|UOM|QTY|Ogi Amt|Remain Amt|CCY|Remark'
                    format='0|0|0|0|0|0|0|0|0' aligns='0|0|0|1|3|3|3|1|0' check='||||||||' 
					editcol='0|0|0|0|0|0|0|0|0'
                    widths='0|2500|4500|800|1000|1400|1400|800|1000' sorting='T' styles='width:100%; height:100%'
        ...
        <tr style="height: 20%">
            <td colspan="10">
                <gw:grid id='idGrid2' header='_pk|Asset Code|Asset Name|UOM|Qty|Ogi Amt|Remain Amt|CCY|Remark'
                    format='0|0|0|0|0|0|0|0|0' aligns='0|0|0|1|3|3|3|1|0' check='||||||||' 
					editcol='0|0|0|0|0|0|0|0|0'
                    widths='0|2500|4500|800|1000|1400|1400|800|1000' sorting='T' 
