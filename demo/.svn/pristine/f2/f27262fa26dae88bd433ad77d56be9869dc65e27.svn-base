...
public partial class rpt_chrm00010_03 : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        string TemplateFile = "rpt_chrm00010_03.xls";
        string TempFile = "../../../../system/temp/rpt_chrm00010_03_" + Session["User_ID"].ToString() + ".xls";
    ...
    = "SELECT * " +
        "FROM (  " +
        "SELECT 1, NVL(DIV_SEQ,99999999) DIV_SEQ,NVL(ORG_SEQ,99999999) ORG_SEQ,NVL(DIV_0,99999999) DIV_0 " +
        ...
        "        START WITH g.pk = e.pk  " +
        "        CONNECT BY PRIOR g.p_pk = g.pk) ORG_seq, " +
        " (SELECT g.ORG_NM  " +
        "              FROM TCO_ORG g  " +
        "             WHERE g.del_if = 0 AND G.ORG_TYPE = '06'  " +
        "        START WITH g.pk = e.pk  " +
        ...
        " FROM THR_EMPLOYEE A,THR_WG_SCH_DETAIL D,tco_org e " +
        ...
        " WHERE A.DEL_IF=0 AND D.DEL_IF=0 AND E.DEL_IF=0  " +
        " AND A.TCO_ORG_PK=E.PK AND A.PK=D.THR_EMP_PK AND A.NATION='01' " +
        ...
        "GROUP BY DIV_0,DIV_SEQ,ORG_SEQ " +
        "UNION ALL " +
        ...
        "FROM THR_MEAL_CHECKING M1   " +
        "WHERE M1.DEL_IF=0   " +
        "AND M1.WORK_DT_OF_SHIFT ='" + p_from_dt + "' AND M1.MEAL_EMP_TYPE='OUT' " +
        ...
        "WHERE M1.DEL_IF=0   " +
        "AND M1.WORK_DT_OF_SHIFT ='" + p_from_dt + "' " +
        "GROUP BY M1.WORK_DT_OF_SHIFT " +
        "UNION ALL " +
        "SELECT 4,0,0, 'OTHER',  " +
        "SUM(NVL(M1.MEAL_QTY,1)*(CASE WHEN M1.MEAL_TIMES = '01' THEN 1 ELSE 0 END)) MEAL_01,  " +
